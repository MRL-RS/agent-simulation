...
public class MrlUnvisitedBuildingLayer extends MrlAreaLayer<Building> {
    ...
    public static Map<EntityID, Set<EntityID>> UNVISITED_BUILDINGS_MAP = Collections.synchronizedMap(new FastMap<EntityID, Set<EntityID>>());
    ...
    protected void paintShape(Building b, Polygon shape, Graphics2D g) {
        if (StaticViewProperties.selectedObject != null) {
            ...
            Set<EntityID> unvisited = null;
            try {
                unvisited = Collections.synchronizedSet(UNVISITED_BUILDINGS_MAP.get(StaticViewProperties.selectedObject.getID()));
